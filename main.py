...
@app.delete(
    "/api/admin/products/{product_id}/comparison",
    response_model=schemas.ProductOut,
    include_in_schema=False,
)
def admin_delete_one_comparison(
    product_id: int,
    url: str,
    x_admin_token: str | None = Header(default=None, alias="X-Admin-Token"),
    db: Session = Depends(get_db),
    ...
    """
    Admin seçtiği TEK karşılaştırma satırını veritabanından siler.
    Ürün sepette kalır. Sonraki fiyat taraması aynı sonucu yeniden bulursa
    compare_and_save_offers tarafından tekrar eklenebilir.
    """
    _verify_admin_token(x_admin_token)
    ...
    if not clean_url:
        raise HTTPException(status_code=400, detail="Karşılaştırma URL'si gerekli.")

    variants = {clean_url, clean_url + "/"}

    deleted = (
        db.query(models.Offer)
        .filter(
            models.Offer.product_id == product.id,
            models.Offer.url.in_(variants),
        )
