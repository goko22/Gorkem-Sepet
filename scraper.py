...
async def scrape_hepsiburada_api(url):
    ...
    # Parse.bot tam ürün URL'si ile çalışıyor.
    # Bazı yeni/paylaşım Hepsiburada linklerinde HBCV kodu URL'de görünmeyebilir;
    # bu yüzden product_code yok diye ürünü reddetme.
    if not product_code:
        print(
            "HB PRODUCT CODE NOT IN URL -> CONTINUE WITH FULL URL",
            flush=True,
        )
    ...
    async with httpx.AsyncClient(
        ...
        # Hepsiburada Render/Playwright tarafında güvenlik sayfasına düşebiliyor.
        # Ürün ekleme işlemini bozmasın diye ana akışta browser basket detector
        # çalıştırmıyoruz. Parse.bot detail/search fiyatı stabil kaynak olarak kalır.
        basket_price = None

        print(
            "HB BASKET BROWSER SKIPPED -> PARSE.BOT STABLE MODE",
            flush=True,
        )
...
async def amazon_primary_price(page):
    """
    Amazon ürün detay sayfasında yalnızca ANA ürün fiyat bloğunu okur.

    Neden ayrı:
    Amazon HTML/JSON-LD içinde taksit, kupon, başka satıcı, liste fiyatı,
    aksesuar veya önerilen ürün fiyatları bulunabiliyor. Generic scraper bunlardan
    birini seçerse örn. 4.199 TL ürün 709,93 TL gibi görünebiliyor.

    Burada body içindeki en sık TL değerini ASLA seçmiyoruz. Öncelik sadece
    aktif ürünün core/apex fiyat alanlarında.
    """
    selectors = [
    ...
    for selector in selectors:
        try:
            ...
            for i in range(count):
                node = locator.nth(i)
                raw = (
                    await node.text_content(timeout=1200)
    ...
    # Bazı Amazon şablonlarında tam fiyat .a-offscreen yerine
    # whole + fraction parçalarıyla geliyor.
    containers = [
    ...
    for selector in containers:
        try:
            ...
            for i in range(count):
                ...
                whole = (
                    await node.locator(".a-price-whole").first.text_content(timeout=900)
                    or ""
                ).strip()
                fraction = (
                    await node.locator(".a-price-fraction").first.text_content(timeout=900)
                    or ""
                ).strip()
    ...
    print("AMAZON PRIMARY PRICE: NOT FOUND", flush=True)
    return None
...
async def scrape_browser(
    ...
    try:
        ...
        if store == "Amazon Türkiye":
            # Amazon JSON-LD/meta içindeki fiyat; taksit, kupon veya başka teklif
            # olabildiği için burada kesinlikle güvenilir kabul edilmez.
            data["price"] = None
            ...
            amazon_price = await amazon_primary_price(page)

            if amazon_price is not None:
                data["price"] = amazon_price
            else:
                # Ana Amazon fiyat bloğu bulunamazsa yanlış bir JSON-LD fiyatını
                # kaydetmek yerine ürün eklemeyi başarısız bırakmak daha güvenli.
                print(
                    "AMAZON PRIMARY PRICE NOT FOUND -> DO NOT USE JSON-LD PRICE",
                    flush=True,
                )
