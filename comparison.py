...
def build_queries(title: str, brand: Optional[str], model: Optional[str]) -> list[str]:
    ...
    if ids:
        ident = ids[0]
        first = " ".join(x for x in (brand, ident, "fiyat satın al") if x)
        out.append(first.strip())

    # Broad but still product-centered query.
    words = [
    ...
    broad = " ".join(([brand] if brand else []) + core + ["fiyat"])
    broad = re.sub(r"\s+", " ", broad).strip()
    if broad and all(norm(broad) != norm(q) for q in out):
        out.append(broad)
    ...
    try:
        async with httpx.AsyncClient(timeout=TAVILY_TIMEOUT, follow_redirects=True) as client:
            ...
            if r.status_code != 200:
                ...
                print("COMPARE TAVILY BODY:", body, flush=True)
                return []

            data = r.json()
            results = data.get("results") or []
            print("COMPARE TAVILY RESULTS:", len(results), "query=", query, flush=True)
            return results
...
def discovery_score(
    ...
    # Search snippets can be noisy. Exact identifier wins heavily.
...
async def discover_candidates(
    ...
    print("COMPARE V6 QUERIES:", queries, flush=True)

    found: dict[str, DiscoveryCandidate] = {}

    for i, query in enumerate(queries):
        rows = await tavily_search(query)
        for row in rows:
            ...
            # Search-level prefilter is intentionally a little looser;
            # the actual store page gets strict verification below.
            if score < 0.70:
                continue
        ...
        # One credit is enough if we already have a healthy candidate pool.
        if len(found) >= 8:
            break
...
async def compare_product(
    ...
    print("=" * 50, flush=True)
    print("COMPARE V6 START:", title, flush=True)
