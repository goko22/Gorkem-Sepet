...
# =========================================================
# GÖRKEM SEPETİ - COMPARISON V12 STABLE
#
# Amaç:
...
def choose_price(text: str, source_price: Optional[float]) -> Optional[float]:
    prices = extract_tl_prices(text)
    if not prices:
        return None

    # Remove absurd tiny/huge numbers relative to source when source price exists.
    if source_price and source_price > 0:
        plausible = [
            p for p in prices
            if source_price * 0.25 <= p <= source_price * 4.0
        ]
        if plausible:
            return plausible[0]
...
async def tavily_search(query: str, include_domains: Optional[list[str]] = None) -> list[dict]:
    ...
    if not key:
        print("COMPARE V12 ERROR: TAVILY_API_KEY yok", flush=True)
        return []
    ...
    try:
        async with httpx.AsyncClient(timeout=TAVILY_TIMEOUT, follow_redirects=True) as client:
            ...
            print("COMPARE V12 TAVILY:", r.status_code, query, flush=True)
            if r.status_code != 200:
                print("COMPARE V12 TAVILY BODY:", r.text[:500], flush=True)
                return []

            rows = (r.json() or {}).get("results") or []
            print("COMPARE V12 RESULTS:", len(rows), query, flush=True)
            return rows
    except Exception as e:
        print("COMPARE V12 TAVILY ERROR:", repr(e), flush=True)
        return []
...
async def compare_product(
    ...
    print("=" * 56, flush=True)
    print("COMPARE V12 STABLE START:", title, flush=True)

    queries = build_queries(title, brand, model)
    print("COMPARE V12 QUERIES:", queries, flush=True)
    ...
    for row in rows:
        ...
        price = choose_price(f"{snippet}\n{raw}", source_price)
        if price is None:
            print("COMPARE V12 NO TL PRICE:", host_of(url), rtitle[:100], flush=True)
            continue
    ...
    print("COMPARE V12 FINAL:", len(final), flush=True)
    for x in final:
        print("COMPARE V12 OFFER:", x.store, x.price, round(x.score, 3), x.url, flush=True)
    print("=" * 56, flush=True)
