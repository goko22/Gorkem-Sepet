...
def strongest_identifiers(text: str, model: Optional[str] = None) -> list[str]:
    ...
    if model:
        ...
        # Ignore marketplace/internal variant IDs.
        # Shopify variant IDs are usually long all-digit values (e.g. 49550669512897).
        # A pure-numeric value is NOT treated as an MPN unless it contains a separator
        # in the original text (handled by model_like_tokens) or is short enough to be
        # plausibly meaningful in the product title itself.
        pure_digits = bool(re.fullmatch(r"\d+", mc))
        bogus_long_numeric = pure_digits and len(mc) >= 9

        if (
            len(mc) >= 5
            and not mc.startswith(("hbcv", "hbc", "b0"))
            and len(mc) <= 40
            and not bogus_long_numeric
        ):
            if re.search(r"\d", m):
                candidates.add(m)
...
def rayban_identity(text: str) -> tuple[Optional[str], Optional[str], Optional[str]]:
    """
    Examples:
      RB2140 901 50      -> rb2140, 901, 50
      0RB2140F 901 52    -> rb2140f, 901, 52
      RB2140 901/5F 50   -> rb2140, 901/5f, 50

    Lens/frame size differences (50/52/54) are allowed as variants.
    Model suffixes (RB2140 vs RB2140F) and color/lens codes are critical.
    """
    n = norm(text)

    model_m = re.search(r"\b0?(rb\d{4}[a-z]?)\b", n)
    if not model_m:
        return None, None, None

    model = model_m.group(1)
    after = n[model_m.end():].strip()

    # First code after model: 901, 901/31, 901/5f, 902, etc.
    color_m = re.search(r"\b(\d{3,4}(?:/[a-z0-9]{1,3})?)\b", after)
    color = color_m.group(1) if color_m else None

    size = None
    if color_m:
        rest = after[color_m.end():]
        size_m = re.search(r"\b(\\d{2})\b", rest)
...
def build_queries(title: str, brand: Optional[str], model: Optional[str]) -> list[str]:
    ...
    # Never let a long numeric storefront/variant ID become the only useful query.
    # If the first query is effectively just digits + TL, drop it.
    cleaned = []
    for q in out:
        qn = norm(q)
        digits = re.sub(r"\D", "", qn)
        letters = re.sub(r"[^a-z]", "", qn)
        if len(digits) >= 9 and len(letters.replace("tl", "")) <= 2:
            continue
        cleaned.append(q)
...
def discovery_score(
    ...
    if looks_like_eyewear(source_title):
        ...
        # For eyewear, never boost a candidate unless the exact frame model matches.
        # Size may differ (50/52/54), but model suffix and color/lens code may not.
        if sm and rm:
            if sm != rm:
                return 0.0
...
async def discover_candidates(
    ...
    print("COMPARE V10 QUERIES:", queries, flush=True)
    ...
    def absorb(rows: list[dict]):
        for row in rows:
            ...
            # Ray-Ban needs explicit exact frame-model identity at discovery time.
            # RB2140 901 50 <-> RB2140 901 54 is OK.
            # RB2140 <-> RB2196, RB2140F, 901/5F are NOT the same product.
            if looks_like_eyewear(title):
                sm, sc, _ = rayban_identity(title)
                cm, cc, _ = rayban_identity(f"{rtitle} {snippet} {url}")
                if sm:
                    if not cm or sm != cm or (sc and cc and sc != cc):
                        print(
                            "COMPARE EYEWEAR-ID REJECT:",
                            host_of(url),
                            rtitle[:120],
                            flush=True,
                        )
...
async def compare_product(
    ...
    print("=" * 50, flush=True)
    print("COMPARE V10 IDENTITY START:", title, flush=True)
