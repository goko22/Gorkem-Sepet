...
# =========================================================
# GÖRKEM SEPETİ - COMPARISON V20 VARIANT-AWARE
#
# Amaç:
...
async def lightweight_product_page(url: str) -> tuple[Optional[float], str]:
    ...
    try:
        ...
        if r.status_code != 200:
            print("COMPARE V20 HTTP SKIP:", r.status_code, host_of(url), flush=True)
            return None, "unknown"

        html = r.text[:MAX_HTTP_PAGE_BYTES]
        return _jsonld_price(html), _html_stock_state(html)
    except Exception as e:
        print("COMPARE V20 HTTP ERROR:", host_of(url), repr(e), flush=True)
        return None, "unknown"
...
async def compare_product(
    ...
    print("=" * 56, flush=True)
    print("COMPARE V20 START:", title, flush=True)

    queries = build_queries(title, brand, model, variant_text)
    print("COMPARE V20 QUERIES:", queries, flush=True)
    ...
    print("COMPARE V20 UNIQUE URLS:", len(unique_rows), flush=True)
    ...
    for row in unique_rows.values():
        ...
        if availability == "out":
            print("COMPARE V20 OUT OF STOCK REJECT:", host_of(url), rtitle[:110], flush=True)
            continue
        ...
        if price is None and http_fallback_budget > 0:
            ...
            if page_stock == "out":
                print("COMPARE V20 HTTP OUT OF STOCK:", host_of(url), rtitle[:100], flush=True)
                continue
        ...
        if price is None:
            print("COMPARE V20 NO PRICE:", host_of(url), rtitle[:100], flush=True)
            continue
    ...
    print("COMPARE V20 FINAL:", len(final), flush=True)
    for x in final:
        print("COMPARE V20 OFFER:", x.store, x.price, round(x.score, 3), x.url, flush=True)
    print("=" * 56, flush=True)
