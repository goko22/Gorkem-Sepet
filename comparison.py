...
def _extract_akakce_offers(product_url: str, html: str, verified_title: str, product_score: float) -> list[MatchResult]:
    ...
    for root in root_nodes:
        for anchor in root.find_all("a", href=True):
            ...
            merchant = _merchant_from_domain(target) or _merchant_from_text(context)
            if not merchant:
                continue
...
async def compare_product(
    ...
    async def one_source(source: str) -> list[MatchResult]:
        try:
            return await search_aggregator(
                source, query, title, brand, model, variant_text,
            )
        except Exception as e:
    ...
    # Kaynak ürün her zaman havuzda kalsın; sepetteki diğer eş ürünleri services.py
    # ayrıca ekler. Bu satır tek başına "en ucuz" iddiası değildir.
