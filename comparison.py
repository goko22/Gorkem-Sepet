...
def build_search_query(title: str, brand: Optional[str], model: Optional[str]) -> str:
    """Mağaza aramasına mümkün olduğunca kısa ve ürün-kimlik odaklı sorgu gönderir."""
    pieces: list[str] = []
    seen: set[str] = set()
    ...
    # Başlıktaki ilk ayırt edici 3-4 kelime genelde marka + seri + sürümdür.
    identity = identity_words(title)
    for w in norm(title).split():
        if w in identity:
            add(w)
        if len(pieces) >= (4 if brand or model_n else 3):
            break

    # 990 Pro gibi sayı + seri kalıbı model kimliğinin parçası olabilir.
    for token in list(series_tokens(title))[:1]:
        add(token)

    # Kapasite / radyatör boyutu gibi ilk kritik teknik değeri de sorguya koy.
    for token in title_spec_tokens(title)[:2]:
        add(token)
