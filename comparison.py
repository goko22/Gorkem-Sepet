...
async def discover_candidates(
    ...
    print("COMPARE V8 TL QUERIES:", queries, flush=True)
    ...
    def absorb(rows: list[dict]):
        for row in rows:
            ...
            # HARD MARKET FILTER:
            # Never even open foreign product pages. This prevents .ae/.co.uk/eBay
            # results from wasting Playwright time and entering the comparison pool.
            if not is_turkish_market_host(url):
                print(
                    "COMPARE FOREIGN REJECT:",
                    host_of(url),
                    url,
                    flush=True,
                )
            ...
            # For unrestricted web search, require explicit Turkish-lira evidence
            # from the search result/snippet. Domain-scoped searches are already
            # Turkish-shop restricted and may omit the price from the snippet.
            if not result_mentions_tl(rtitle, snippet):
                print(
                    "COMPARE NO-TL REJECT:",
                    host_of(url),
                    rtitle[:120],
                    flush=True,
                )
...
async def compare_product(
    ...
    print("=" * 50, flush=True)
    print("COMPARE V8 TL START:", title, flush=True)
