...
# =========================================================
# GÖRKEM SEPETİ - COMPARISON V19 SEARCH RECOVERY
#
# Amaç:
...
def model_tokens(text: str) -> set[str]:
    ...
    for t in re.findall(r"\b[a-z0-9][a-z0-9._+-]{3,}\b", n):
        ...
        # simple spec values: 360mm, 310hz, 1ms, 32000dpi, 32gb...
        m = re.fullmatch(r"(\d+(?:[.,]\d+)?)([a-z]+)", t)
        if m and m.group(2) in spec_units:
            continue
...
async def lightweight_product_page(url: str) -> tuple[Optional[float], str]:
    ...
    try:
        ...
        if r.status_code != 200:
            print("COMPARE V19 HTTP SKIP:", r.status_code, host_of(url), flush=True)
            return None, "unknown"

        html = r.text[:MAX_HTTP_PAGE_BYTES]
        return _jsonld_price(html), _html_stock_state(html)
    except Exception as e:
        print("COMPARE V19 HTTP ERROR:", host_of(url), repr(e), flush=True)
        return None, "unknown"
...
async def compare_product(
    ...
    print("=" * 56, flush=True)
    print("COMPARE V19 START:", title, flush=True)

    queries = build_queries(title, brand, model)
    print("COMPARE V19 QUERIES:", queries, flush=True)
    ...
    print("COMPARE V19 UNIQUE URLS:", len(unique_rows), flush=True)
    ...
    for row in unique_rows.values():
        ...
        if availability == "out":
            print("COMPARE V19 OUT OF STOCK REJECT:", host_of(url), rtitle[:110], flush=True)
            continue
        ...
        if price is None and http_fallback_budget > 0:
            ...
            if page_stock == "out":
                print("COMPARE V19 HTTP OUT OF STOCK:", host_of(url), rtitle[:100], flush=True)
                continue
        ...
        if price is None:
            print("COMPARE V19 NO PRICE:", host_of(url), rtitle[:100], flush=True)
            continue
    ...
    # Same store + same URL only once. Different listings from same marketplace
    # remain visible because they can be different sellers/prices.
    ...
    print("COMPARE V19 FINAL:", len(final), flush=True)
    for x in final:
        print("COMPARE V19 OFFER:", x.store, x.price, round(x.score, 3), x.url, flush=True)
    print("=" * 56, flush=True)
